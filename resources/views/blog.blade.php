...
<div class="row">
	<div class="col-md-8">
		...
		<div class="row">
			...
			<!-- post -->
			@foreach ($data as $new_posting)
			
			<div class="col-md-6">
				<div class="post">
					<a class="post-img" href=" {{ route('blog.isi', $new_posting->slug) }} "><img src=" {{ $new_posting->gambar }} " alt=""></a>
					<div class="post-body">
						<div class="post-category">
							<a href="#"> {{ $new_posting->category->name }} </a>
						</div>
						<h3 class="post-title"><a href=" {{ route('blog.isi', $new_posting->slug) }} "> {{ $new_posting->Judul }} </a></h3>
						<ul class="post-meta">
							<li><a href="#"> {{ $new_posting->users->name }} </a></li>
							<li> {{ $new_posting->created_at->diffforHumans() }} </li>
						</ul>
					</div>
				</div>
			</div>
